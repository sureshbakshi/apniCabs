...
const OTPForm = ({ heading, successHandler, formFields, formSchema, formMutation, getOTPPayloadKeys = [], initialState, submitBtnLabel, additionalOTPPayload = {}, verifyOTPMutation, formPayloadKeys, additionalVerifyOTPPayload = {}, scrollRef }) => {
    ...
    useEffect(() => {
        ...
        if (global.requestIdleCallback) {
            handle = global.requestIdleCallback(fetchCities);
        } else {
            handle = setTimeout(fetchCities, 200);
        }
    ...
    const onSubmit = React.useCallback((formData) => {
        ...
        Keyboard?.dismiss()
    }, [getOTPPayloadKeys, submitHandler, additionalOTPPayload, formPayloadKeys]);

    const getOTP = React.useCallback((deviceCode) => {
        const OTPPayload = extractKeys(payload, getOTPPayloadKeys)

        submitHandler({ ...OTPPayload, ...(additionalOTPPayload && additionalOTPPayload) });//add device code
    }, [payload, getOTPPayloadKeys, submitHandler, additionalOTPPayload]);

    const errorHandler = React.useCallback(({ data }) => {
    ...
    const callbackFunctions = React.useMemo(() => ({
        getOTP: getOTP,
        successHandler: successHandler,
        errorHandler: errorHandler,
        verifyOTPMutation: verifyOTPMutation
    }), [getOTP, successHandler, errorHandler, verifyOTPMutation]);

    const cityTypesFormat = React.useMemo(() => cities
