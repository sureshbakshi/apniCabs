...
const Card = (item) => <View style={FindRideStyles.card} key={item.id}>
    <Timeline />
    <View style={FindRideStyles.cardtop}>
        <View style={FindRideStyles.left}>
            <ImageView
                source={images[`captain${item.profile_avatar}`] || images[`captain0`]}
                style={[styles.avatar]}
            />
        </View>
    ...
    <View style={FindRideStyles.cardBottom}>
        <View style={FindRideStyles.left}>
            {item?.distance_away && <Text style={[styles.text, styles.bold]}>{item.distance_away} away </Text>}
        </View>
        <View style={FindRideStyles.middle}>
